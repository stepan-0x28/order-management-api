...
from app.features.users.schemas import UserIn, UserOut, UserPersonal
...
@router.put('/me')
async def update_current_user(current_user: Annotated[User, Depends(get_current_user)],
                              user_personal: Annotated[UserPersonal, Depends(FormMaker(UserPersonal))],
                              user_service: Annotated[UserService, Depends(get_user_service)]):
    await user_service.change_data(current_user, user_personal)
